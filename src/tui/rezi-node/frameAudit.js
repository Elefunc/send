...
function hashFnv1a32(bytes, end) {
...
function sliceHex(bytes, start, end) {
...
function readHeaderU32(bytes, offset) {
...
function decodeOpcodeHistogram(bytes) {
...
const FRAME_AUDIT_LOG_PATH = readEnv("REZI_FRAME_AUDIT_LOG") ?? (FRAME_AUDIT_ENABLED ? "/tmp/rezi-frame-audit.ndjson" : null);
...
function readContextRoute() {
    const g = globalThis;
    if (typeof g.__reziFrameAuditContext !== "function")
        return null;
    try {
...
export function maybeDumpDrawlistBytes(scope, stage, frameSeq, bytes) {
    if (!FRAME_AUDIT_ENABLED)
        return;
    if (FRAME_AUDIT_DUMP_DIR === null)
        return;
    if (FRAME_AUDIT_DUMP_MAX <= 0)
        return;
    if (frameAuditDumpCount >= FRAME_AUDIT_DUMP_MAX)
        return;
    const route = readContextRoute();
    if (FRAME_AUDIT_DUMP_ROUTE !== null && route !== FRAME_AUDIT_DUMP_ROUTE)
        return;
    try {
...
export function drawlistFingerprint(bytes) {
...
export function createFrameAuditLogger(scope) {
    if (!FRAME_AUDIT_ENABLED) {
        return Object.freeze({
            enabled: false,
            emit: () => { },
    ...
    return Object.freeze({
        enabled: true,
        emit: (stage, fields = Object.freeze({})) => {
            try {
                const context = typeof g.__reziFrameAuditContext === "function" ? g.__reziFrameAuditContext() : null;
...
//# sourceMappingURL=frameAudit.js.map